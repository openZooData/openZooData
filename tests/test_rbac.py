...
def rbac_setup(admin_token):
    ...
    try:
        ...
        # Dedizierter Endpoint — harter DELETE, idempotent.
        requests.delete(f"{BASE_URL}/api/v1/admin/test-fixtures/rbac",
                        headers=h)
...
def test_zoo_b_token_rejected_for_zoo_a_publish(rbac_setup):
    ...
    # 429 = Rate-Limit (zu viele Requests kurz hintereinander)
    # Alle bedeuten: kein erfolgreicher Publish — Test ist bestanden.
    assert resp.status_code in (403, 409, 429), \
